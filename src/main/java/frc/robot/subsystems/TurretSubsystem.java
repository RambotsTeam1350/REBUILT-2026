...
public class TurretSubsystem extends SubsystemBase {
    ...
    private int dashboardTickCount = 0;
    ...
    public void periodic() {
        ...
        if (++dashboardTickCount >= 25) {  // ~0.5s at 50 Hz, on the main thread (no data races)
            dashboardTickCount = 0;
            SmartDashboard.putNumber("angle turret will turn to", getRawAngle());
            SmartDashboard.putNumber("position turret will turn to", turretDegreesAndEncoderUnits(getRawAngle()));
